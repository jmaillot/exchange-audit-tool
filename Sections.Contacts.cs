    ...
    internal static class SectionsContacts
    {
        ...
        private static string BuildRecipientScript(AuditSelection sel, ScriptContext ctx, string baseCmdlet, string label, string[] groupKeys)
        {
            ...
            if (needResolver)
            {
                ...
                sb.AppendLine("            try { $r = " + getRecip + " -Identity $key -ErrorAction Stop; if ($r) { if ($r.DisplayName) { $smtp = [string]$r.DisplayName } elseif ($r.PrimarySmtpAddress) { $smtp = [string]$r.PrimarySmtpAddress } } } catch { }");
