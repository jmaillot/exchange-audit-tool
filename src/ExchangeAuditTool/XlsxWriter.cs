...
namespace ExchangeAuditTool
{
    ...
    internal static class XlsxWriter
    {
        ...
        private const string ExtPropsRelType = "http://schemas.openxmlformats.org/package/2006/relationships/metadata/extended-properties";
        ...
        public static XlsxWriteResult WriteFromCsv(string csvPath, string xlsxPath, string sheetName)
        {
            ...
            try
            {
                ...
                {
                    // NOTE: [Content_Types].xml is generated by Package itself
                    // on close - writing it by hand collides with that.
                    WriteDocProps(package);
                    PackagePart workbook = WriteWorkbookPart(package, sheetName);
                    WriteStyles(package);
                    ...
                    workbook.CreateRelationship(sheet.Uri, TargetMode.Internal, WorksheetRelType, "rId1");
                    workbook.CreateRelationship(new Uri("/xl/styles.xml", UriKind.Relative), TargetMode.Internal, StylesRelType, "rId2");
                    package.CreateRelationship(new Uri("/xl/workbook.xml", UriKind.Relative), TargetMode.Internal, OfficeDocumentRelType, "rId1");
                    package.CreateRelationship(new Uri("/docProps/core.xml", UriKind.Relative), TargetMode.Internal, CorePropsRelType, "rId2");
                    package.CreateRelationship(new Uri("/docProps/app.xml", UriKind.Relative), TargetMode.Internal, ExtPropsRelType, "rId3");
                }
        ...
        private static PackagePart WritePart(Package package, string path, string contentType, Action<XmlWriter> body)
        {
        ...
        private static PackagePart WriteWorkbookPart(Package package, string sheetName)
        {
            return WritePart(package, "/xl/workbook.xml", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet.main+xml", delegate (XmlWriter w)
            {
